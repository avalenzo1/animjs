...
const Stage = memo(forwardRef(
  ({ animRef, onReady, onNewFrame }: StageProps, ref) => {
    ...
    const renderScene = useCallback(
      ...
      [animRef, getCurrentFrame, getNextFrames, getPreviousFrames, renderOnion],
    );
    ...
    useEffect(() => {
      function renderStage() {
        if (!stageCtxRef.current || !stageCanvasRef.current || !animRef.current)
          return;
        ...
        stageCtx.arc(
          mouseRef.current.x,
          mouseRef.current.y,
          animRef.current.brush.size / 2,
          0,
          2 * Math.PI,
        );
        stageCtx.fillStyle = "rgba(255, 0, 0, 0.8)";
        stageCtx.fill();
    ...
    function handlePointerDown(e: React.MouseEvent<HTMLCanvasElement>) {
      ...
      switch (animRef.current.mode) {
        ...
        case E_Mode.BRUSH:
          ...
          let newAnimObject = new AnimStroke({ brush: animRef.current.brush });
    ...
    useImperativeHandle(ref, () => ({
      ...
      exportVideo() {
        ...
        recorder.onstop = async () => {
          ...
          animRef.current.isExporting = false;
